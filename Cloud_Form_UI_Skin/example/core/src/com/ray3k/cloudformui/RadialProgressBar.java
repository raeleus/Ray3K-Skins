...
public class RadialProgressBar extends Widget implements Disableable {
    ...
    public RadialProgressBar(float min, float max, float stepSize, Skin skin) {
        this(min, max, stepSize, skin.get("default", CircularProgressBarStyle.class));
    }

    public RadialProgressBar(float min, float max, float stepSize, Skin skin, String styleName) {
        this(min, max, stepSize, skin.get(styleName, CircularProgressBarStyle.class));
    }
     ...
     */
    public RadialProgressBar(float min, float max, float stepSize, CircularProgressBarStyle style) {
        startAngle = 90.0f;
        cw = true;
    ...
    public void setStyle(CircularProgressBarStyle style) {
        if (style == null) {
    ...
    public void draw(Batch batch, float parentAlpha) {
        ...
        if (knob != null) {
            if (!MathUtils.isZero(percent)) {
                if (!cw) {
                    knob.draw(batch, getX(), getY(), getWidth(), getHeight(), startAngle, percent * 360.0f);
                } else {
                    knob.draw(batch, getX(), getY(), getWidth(), getHeight(), 360.0f - percent * 360.0f + startAngle, percent * 360.0f);
                }
            }
        }
    }
    ...
    public static class CircularProgressBarStyle {
        ...
        public CircularProgressBarStyle(Drawable background, RadialDrawable knob) {
            this.background = background;
            this.knob = knob;
        }

        public CircularProgressBarStyle(CircularProgressBarStyle style) {
